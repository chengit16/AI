...
全量重建始终从当前已发布文档创建新构建，健康旧索引在新构建提交前继续服务。永久失败或三次人工恢复耗尽的不可见 Chunk 可以清理，仍可恢复的死信必须保留。`P2-10` 已通过受控运营工作台接入巡检、全量重建和清理入口；请求必须经过权限、固定确认词、幂等、审计和 Outbox 门禁，再由 Indexing Worker 租约认领并执行有限重试。不得用手工改表代替维护应用层。
