...
def test_registry_classifies_credentials_and_dependent_facts() -> None:
    ...
    assert registry.registry_version == 22
    ...
    assert policies["document_accesses"].purge is True
    ...
    assert policies["l3_isolation_resource_profiles"].classification == "governance"
    assert policies["l3_isolation_resource_profiles"].purge is False
    assert policies["l3_isolation_migration_checkpoints"].purge is False
    assert policies["l3_isolation_recovery_records"].purge is False
    ...
    for table_name in (
        ...
        assert policies[table_name].classification == "retained"
        assert policies[table_name].export is False
        assert policies[table_name].purge is False
