...
import scripts.run_p6a06_acceptance as acceptance
from scripts.run_p6a06_acceptance import CommandResult, execute_acceptance, load_manifest

ROOT = Path(__file__).parents[1]
MANIFEST_PATH = ROOT / "tests/fixtures/e2e/p6a06-v1.json"
MANIFEST_SCHEMA_PATH = ROOT / "contracts/acceptance/stage-6-a-acceptance.v1.schema.json"
EVIDENCE_SCHEMA_PATH = ROOT / "contracts/acceptance/stage-6-a-acceptance-evidence.v1.schema.json"
...
def test_p6a06_manifest_and_evidence_schemas_are_valid() -> None:
    ...
    assert load_manifest(ROOT, MANIFEST_PATH)["acceptance_id"] == "p6a06-v1"


def test_p6a06_manifest_covers_capabilities_and_unique_real_tests() -> None:
...
def test_p6a06_rejects_relaxed_execution_policy(
    ...
    path = tmp_path / "invalid-p6a06.json"
    path.write_text(json.dumps(manifest, ensure_ascii=False), encoding="utf-8")
    with pytest.raises(ValidationError):
        acceptance._validate_document(manifest, MANIFEST_SCHEMA_PATH)


def test_p6a06_success_writes_schema_valid_evidence(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    evidence_path = tmp_path / "evidence/p6a06-latest.json"
...
def test_p6a06_preflight_failure_is_fail_closed(tmp_path: Path) -> None:
    ...
    code, evidence = execute_acceptance(
        ROOT, MANIFEST_PATH, tmp_path / "evidence/p6a06.json", runner=runner
    )
    assert code == 1
