"""提供 P5-02 真实 PostgreSQL 测试的隔离 Schema Fixture。"""
...
from tests.support.p502_quality import QualityHarness
...
def quality_database() -> Iterator[QualityHarness]:
    """迁移独立 Schema，并在模块结束后清理全部合成事实。"""

    database_url = os.environ.get("AI_PLATFORM_TEST_DATABASE_URL", DEFAULT_DATABASE_URL)
    schema = f"p502_test_{uuid4().hex}"
