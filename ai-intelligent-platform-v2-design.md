...
| 文档版本 | V2.9 |
| 文档状态 | 阶段 1～4 已完成并冻结，阶段 5 外部门禁并行受阻，阶段 6 第一版已启动 |
| 适用范围 | 个人版、企业版、本地验证、后续 SaaS 与私有化部署 |
| 核心定位 | 以工作空间为隔离边界，以知识库为数据底座，以 Agent 为能力载体，以服务发布为产品出口 |
| 本次调整 | 正式启动阶段 6“内容工作台与产品化体验”，冻结第一版阶段 A+B、知识组织关系、企业发布审批和迁移边界；在线文档编辑器与多人协作后置 |
...
- 阶段 6 第一版不建设在线正文、富文本编辑、自动保存、文档 AI 写回或多人实时协作。
- 阶段 6 第一版不开放匿名分享、公开助手、知识广场、标准 MCP 出口或支付交易。
...
一键启动通过 `AI_PLATFORM_MIN_FREE_DISK_GB` 配置最低可用磁盘，默认保持 50 GB。资源不足但仅执行小规模功能开发时，可由操作者在当前进程或本地 `.env` 中显式降低阈值；降低值必须记录到阶段报告，只改变本地启动门禁，不改变套餐额度、正式发布基线或容量认证结论。
...
| 阶段 6 | 内容工作台与产品化体验 | 个人文件型知识管理、企业知识治理和企业大脑简版 | `P6A-01` 范围冻结；非模型节点可与阶段 5 并行 | 阶段 A+B 功能、隔离、迁移和联合验收通过 |
| 条件验收 | 完成容量认证 | 百万 Chunk、20/50 问答并发、500 条 SSE、100 QPS 报告 | 获得合适压测资源 | `capacity_certification=passed`；不影响本地 MVP 状态 |
| 项目后置 | 按价值扩展内容、AI 和数据来源 | 在线编辑器、协作、公开生态、MCP、支付、LLM Grading、多模态和具体连接器 | 第一版稳定且有明确业务指标 | 每项能力独立立项、灰度和验收 |
...
**目标**：把现有平台底座整理为可持续使用的个人文件型知识管理和企业知识治理产品，补齐页面、组织关系、状态反馈、聚合查询和完整操作闭环。

**依赖**：`P6A-01` 先冻结第一版范围与领域关系。目录、文件、成员、分类、审批页面和合成数据机制可与阶段 5 外部门禁并行；AI 助手和 AI 企业大脑对外开通仍等待真实质量、成本、隔离和数据政策门禁。

**阶段 A 建设内容**：

1. 工作台、最近文档、个人统计、快捷上传和全局搜索。
2. 文件夹树、标签、收藏、回收站、恢复和保留期清理。
3. 文件上传、新版本、移动、解析详情、Chunk/索引状态、发布、重试、批量操作和授权下载。
4. AI 助手知识范围、临时附件、快捷指令和会话归档；不向在线正文写回模型结果。

**阶段 B 建设内容**：

1. 企业控制台、资料、知识统计、趋势和快捷入口。
2. 邀请链接、成员、部门、岗位、角色、停用与移除的一体化团队管理。
3. 企业分类、团队知识域、成员/部门范围和文档/知识库显式绑定。
4. 文档发布请求、多级审批、驳回、撤回、过期和审批期间撤权复核。
5. 权限矩阵、影响成员、审计详情与异步导出状态。
6. 只读取当前企业内获权团队知识域的 AI 企业大脑简版。

**领域与安全边界**：

- `KnowledgeBase → Document → DocumentVersion → DocumentPublication` 继续是内容、版本和发布的唯一事实链。
- `KnowledgeFolder` 只组织文档，`EnterpriseCategory` 只表达治理分类，`TeamKnowledgeDomain` 只表达企业协作与 RAG 范围；三者不复制文档、对象、Chunk 或索引。
- 企业审批只控制不可变 `DocumentVersion` 的发布动作；上传、解析和索引不等待审批。
- AI 企业大脑范围为团队知识域声明集合、当前资源级 PDP 授权和当前已发布索引的交集；失败或空交集不得回退到整个企业空间。
- 所有关系、缓存、检索、引用、导出和统计使用可信 `RequestContext.workspace_id`；任意跨空间命中阻断整次操作并告警。

**第一版非目标**：在线正文、富文本、自动保存、文档 AI 写回、实时协作、公开助手、知识广场、标准 MCP 出口和支付。在线编辑器统一记录为 `POST-EDITOR-01`，完成阶段 A+B 后再独立评审。

**验收标准**：

- 个人空间能够完成创建目录、上传、新版本、解析索引、发布、移动、标签、收藏、删除恢复、搜索、问答、引用和审计闭环。
- 企业空间能够完成成员治理、分类与知识域、发布审批、权限审计和企业问答闭环。
- 目录、分类和知识域调整不修改文档版本、对象、Chunk 或当前发布事实；发布审批通过前后变化可从同一事实链追溯。
- 个人/企业、部门、停用成员和跨空间正反向测试，以及空库/存量库迁移、恢复、桌面/移动浏览器和统一门禁通过。
- 阶段 A+B 全部节点完成并形成 `stage-6-complete` 标签；真实 AI 开通条件没有满足时继续准确显示受阻，不把机制验收写成真实质量通过。
...
flowchart LR
    ...
    P2 --> P5
    P1G --> P6A["阶段 6A<br/>个人知识工作台"]
    P6A --> P6B["阶段 6B<br/>企业知识治理"]
    P5 -."约束 AI 对外开通".-> P6A
    P5 -."约束 AI 对外开通".-> P6B
    P6B --> PX["项目后置<br/>编辑器、协作、公开生态、MCP、支付及增强 AI"]
    P1G -."获得压测资源后".-> CAP["独立容量认证<br/>不阻塞本地 MVP"]
    P2 -."达到性能触发条件".-> GO["条件演进<br/>Go 接入层 / 运行层"]
    P4 -."Runtime 稳定且达到触发条件".-> GO
```

原平台底座关键路径是阶段 0 → 1A → 1B → 1C → 1D → 1E → 1G。里程碑 1F 在 1C 后即可启动，可以与 1D、1E 的非集成工作并行，但必须在 1G 前完成。阶段 2 和阶段 3 可以并行，阶段 4 同时等待阶段 2 和阶段 3；阶段 6 的非模型产品节点可与阶段 5 外部输入收口并行，AI 能力对外开通仍受阶段 5 门禁约束。
...
阶段 6 第一版 A+B 稳定，并满足相应阶段 5 对外开通门禁后，再按以下顺序评估后置能力：

1. 先独立评审阶段 C 的连接器、术语、场景、RAG 评估、工作流模板和办公 Agent，不把整组能力自动视为第一版续作。
2. 单独评审 `POST-EDITOR-01` 的在线正文、富文本、自动保存、历史、格式兼容和文档 AI 写回；只有编辑器稳定后才评审实时协作。
3. 将 `LLM Grading` 先用于离线评估，验证收益和成本后，再对低置信度或高风险问题灰度启用在线评分。
4. 选择一个业务价值明确的 `DataSource`，验证身份映射、增量同步、权限回查、来源策略和故障隔离；不同连接器逐个验收，不批量铺开。
5. 完成图片对象存储、图片权限、安全检测和成本核算后，再启用多模态图片问答。
6. 公开助手、知识广场、标准 MCP 出口和支付分别完成安全、审核、商业和责任评审后独立立项。
7. 每项后置能力必须独立设置功能开关、工作空间级配额、审计字段、质量指标和回滚方案。

这样安排不会改变当前项目的核心边界：在线编辑器、协作、公开生态、标准 MCP、支付、多源连接器、LLM Grading 和图片问答都不阻塞阶段 6 第一版 A+B 交付。
...
当前机器可用磁盘会随 Docker 镜像和开发数据变化；启动脚本必须按实际值检查。小规模功能开发可通过 `AI_PLATFORM_MIN_FREE_DISK_GB` 显式调整门禁并记录实际值，但必须通过配额和数据集配置限制文档、Chunk、日志和备份规模。若未来执行百万 Chunk 容量认证，再准备外置 NVMe SSD 或独立 Linux 机器；当前不设置压测机采购、准备或交付时间。
...
前端布局、动态菜单、个人/企业空间导航、响应式断点、可访问性、加载/错误状态和阶段 1 完成态统一记录在 [`docs/design/ui-ux-baseline.md`](./docs/design/ui-ux-baseline.md)。`P1G-04` 已固定森林绿主品牌色、深绿导航、克制蓝绿色强调色、系统无衬线字体和全局焦点行为；暗色模式仍需单独立项和完整状态验收。UI/UX 变更继续遵循节点验收、文档同步和独立 Git 提交规则。
