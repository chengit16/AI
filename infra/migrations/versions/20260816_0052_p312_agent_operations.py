"""激活 P3-12 AgentRelease 运营权限、菜单快照和 Trace 聚合索引。"""
...
PAGE_MENU_ID = "82000000-0000-4000-8000-000000000219"
ACTION_MENU_ID = "82000000-0000-4000-8000-000000000220"
...
def upgrade() -> None:
    """为既有 Owner 和当前菜单发布追加 Registry 21 运营入口。"""
...
def downgrade() -> None:
    """仅在不存在自定义授权时恢复 Registry 20 菜单并移除本节点索引。"""
    ...
    op.execute(
        ...
        ).bindparams(menu_id=ACTION_MENU_ID, api_resource_id=API_RESOURCE_ID)
    )
    op.execute(
        sa.text(
            f"""
...
def _grant_owner_permission(schema: str) -> None:
    """个人和企业 Owner 默认可读，其他成员只能经自定义角色显式授权。"""
...
def _register_binding(schema: str) -> None:
    op.execute(
        sa.text(
            ...
            ) VALUES (CAST(:menu_id AS uuid), CAST(:api_resource_id AS uuid), 'query')
...
def _publish_upgraded_menu_snapshots(schema: str) -> None:
    """复制每个当前发布并追加运营菜单，历史与管理员自定义项保持不变。"""
...
def _insert_upgraded_menu_release(
    ...
    bindings.append(_operations_snapshot_binding())
...
def _reject_custom_grants(schema: str) -> None:
    """保留用户自定义授权事实；发现非系统 Owner 授权时拒绝破坏性降级。"""
    ...
    if int(count or 0) > 0:
        raise RuntimeError("存在自定义 Agent 运营授权, 拒绝降级以避免权限事实丢失")


def _operations_snapshot_menus() -> list[dict[str, object]]:
    return [
        {
            "menu_id": PAGE_MENU_ID,
...
def _operations_snapshot_binding() -> dict[str, str]:
    return {
        "menu_id": ACTION_MENU_ID,
...
def _upgrade_release_id(workspace_id: UUID) -> UUID:
    return uuid5(NAMESPACE_URL, f"ai-platform:p312-menu:{workspace_id}")
