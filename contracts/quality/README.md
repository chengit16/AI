...
本目录冻结阶段 5 质量样本闭环及六层评估的来源类型、授权投影、正文摘要、版本、删除传播、评估身份和问题归因约束。契约只描述可进入质量数据集与评估事实的机制证据，不给出真实模型质量结论。
...
`quality-evaluation-baseline.v1.json` 固定检索、引用、模型、Prompt、工作流和工具六层顺序、样本下限、最低得分、评估器版本、低基数原因码和全合成通过观测。其 `evidence` 只用于验证摘要边界，不得进入运行、层结果或样本结果表。
...
- 六层评估固定使用 `deterministic-layered-v1`；数据集、服务、AgentRelease、运行配置或评估器身份漂移一律失败关闭。
- 任一层样本不足、失败、超时、跳过或低于阈值时均不得通过；越权、受限字段泄漏、无效引用和未授权工具调用不能被其他高分样本抵消。
- `LLM Grading` 在本契约中固定为关闭，阶段 5 不把主观在线评分模型作为必需门禁。
...
```bash
uv run pytest tests/test_p502_quality_contract.py tests/test_p503_quality_evaluation_contract.py tests/contract/test_contracts.py
```
