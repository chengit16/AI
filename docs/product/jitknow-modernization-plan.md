...
> 当前进度：`P6A-01`～`P6A-06` 与 `P6B-01`～`P6B-06` 已提交完成；`P6B-07` 联合验收已通过，等待节点提交与阶段关闭
...
`P6B-05` 已完成实现、验证与独立功能提交 `111b585`。角色治理聚合统一返回系统/自定义角色、可信 `role_version`、授权矩阵、影响成员、可编辑状态、字段遮罩和安全等级；权限整体替换使用乐观锁，系统角色和停用角色保持只读，授权、版本、审计与 Outbox 同事务提交。审计列表不返回自由属性，详情按白名单投影并递归脱敏；异步导出在创建时冻结筛选、字段权限与时间上界，由 Worker 以租约、有限重试和幂等语义生成确定性 JSONL SHA-256 安全摘要。Registry 已升至 35，字段策略 Registry 升至 3，生命周期 Registry 升至 22，数据库 Revision 为 `20260903_0080`。完整 `./scripts/verify` 的 Web 114/114、Python 1064/1064、mypy strict 825 个源文件及全部工程门禁通过；`./platform doctor` 13/13 通过。全合成企业真实完成系统角色只读、自定义角色影响成员、陈旧版本 `ROLE_CONFLICT` 自动刷新、成功保存、审计 URL 筛选恢复、详情脱敏和异步导出完成；桌面 `1440×900` 与移动 `390×844` 无页面级横向溢出，宽表只在局部滚动，浏览器 warning/error 为 0。当前节点为 `P6B-06` AI 企业大脑简版。

`P6B-06` 已完成企业大脑代码、真实运行和最终统一门禁验证，功能提交为 `715a319`。企业大脑以 `conversation_kind='enterprise_brain'` 隔离普通助手会话，创建会话冻结团队知识域及 RAG 策略版本，每次提问重新鉴权并冻结当前有效知识库范围；空范围、撤权或依赖失败不回退全企业。页面复用既有 SSE、取消、来源和反馈链路，增加活动/归档会话、近 30 天调用统计、近期活动及不可变 Markdown 报告；报告引用数量来自当前授权 Retrieval Evidence，不解析模型正文。普通企业成员的全局壳层改用低敏管理员资格接口，不再调用管理员模型供应商列表探测菜单资格，而管理员业务接口仍保持 403 安全边界。Revision 为 `20260903_0081`，Registry 为 38；P6B-06 PostgreSQL 8/8、管理员资格/Registry/契约专项 45/45、Web 122/122 和 13/13 平台诊断已通过。全合成数据的桌面 `1440×900` 与移动 `390×844` 页面均无全局横向溢出，来源、反馈、报告详情、归档和移动导航可用，API 失败及应用控制台 warning/error 均为 0。Local Mock 仍只证明本地机制，真实供应商对外开通继续受阶段 5 门禁约束。

`P6B-07` 已使用冻结的 `p6b07-v1` 全合成企业清单完成阶段 B 联合验收。验收入口依次执行前置诊断、12 个冻结 PostgreSQL 节点和恢复诊断，覆盖企业统计与字段裁剪、团队成员生命周期、知识域与 PDP 交集、跨空间引用拒绝、两级发布审批原子性、撤权失败关闭、审计详情与 Worker 导出、企业大脑闭环及 Migration 降级保护；场景 12/12、两次平台诊断各 13/13 通过。最终原样 `./scripts/verify` 的 Web 122/122、Python 1082/1082、mypy strict 831 个源文件及全部工程门禁通过。双视口结论复用 P6B-01～P6B-06 已完成的真实合成数据验收，联合验收器不驱动浏览器；本轮 macOS 锁屏未被误写为新的浏览器执行证据。
