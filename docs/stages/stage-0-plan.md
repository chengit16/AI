...
flowchart TD
    ...
    P014 --> P015["P0-15 后端代码规范"]
    P015 --> P016["P0-16 工程门禁与 AI 安全基线"]
    P016 --> P012
    P009 --> P012
    P010 --> P012
    P011 --> P012
...
| `P0-11` | 合成评估集、越权、字段泄漏、失败恢复、Prompt Injection、知识投毒、引用伪造和数据外泄样本 | 数据全部标记为合成，安全用例覆盖 RAG 信任边界并自动化通过 |
...
| `P0-15` | Python 后端模块、接口、权限、事务、数据、Worker、SSE、测试和未来 Go 演进规范 | 与模块化单体、数据写入权和核心契约一致，仓库入口可使后续开发强制执行，阶段 1 自动门禁顺序明确 |
| `P0-16` | `./scripts/verify`、架构检查、契约兼容、仓库安全、ADR 和 AI/RAG 威胁模型 | 检查器具有反例测试并通过全量门禁；当前能力与阶段 1/P0-12 后续项边界清晰 |
...
- `docs/governance/backend-code-standards.md`：Python 后端模块、安全、数据、可靠性和质量门禁。
- `docs/governance/engineering-guardrails.md`：本地与未来 CI 共用的工程门禁和失败处理。
- `docs/governance/architecture-decisions.md`：关键架构决策的触发、评审和演进规则。
- `docs/security/ai-rag-threat-model.md`：AI/RAG 信任边界、攻击分类和安全验收基线。
