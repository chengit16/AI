...
- 当前阻塞：受限环境直接运行正式入口时 `check_runtime` 报告 Docker Engine 不可访问；2026-08-17 08:11 CST 再次原样请求 `./platform accept-stage-4-tools` 提权，仍未进入 Docker、测试或项目代码执行，审批服务返回 `503 Service Unavailable`；08:09 CST 原样请求 `./scripts/verify` 提权也在项目代码执行前被同一审批服务故障拒绝。系统明确禁止通过间接命令绕过，当前仍没有生成 P4-12 正式证据，不能把审批服务或沙箱限制误报为产品通过或失败；继续执行这两个原样入口需要审批服务恢复或用户在提示中直接人工批准。
