...
- 状态：实现已完成并按用户指令直接提交；正式联合演练证据与统一门禁仍待审批服务恢复后补验，节点不得据此标记完成。
- 已实现范围：新增 `p4-12-v1` 冻结清单，使用八个唯一真实 PostgreSQL 测试节点覆盖并发重复投递、Adapter 超时失败、提交后响应丢失、凭证轮换/撤销、确认与审批过期、取消和迟到成功竞态、Worker 重启租约恢复及跨空间绕过；清单同时明确纳入成功、失败、超时、取消、恢复和拒绝六类样本。
- 执行与证据：新增 `./platform accept-stage-4-tools`，固定执行前置 `./platform doctor`、一次 pytest/JUnit 和恢复后 `./platform doctor`，任一诊断、命令、测试缺失、失败、跳过或 JUnit 异常都会使整次演练失败关闭；Shell 包装层由回归测试把整个函数体精确冻结为 6 条非空行，只能依次运行 `check_runtime`、`ensure_environment`、`export_local_service_settings` 和唯一正式执行器及其冻结清单、证据路径参数，不能追加其他命令或扩大执行范围。六项参数化反例同时锁住前置/恢复命令、前置失败停止、始终恢复诊断、失败关闭模式和正式证据路径，任一字段放宽都会在清单校验阶段拒绝。清单还要求完整测试节点和 JUnit 裸函数名各自唯一，将每类能力与场景 ID、P4-01 来源和 PostgreSQL 节点三元绑定，并精确绑定每个能力的样本类别与验证不变量；即使全局总并集仍完整，单场景虚增样本类别或减少自身验证责任也会失败关闭。清单固定 8 个场景对应 9 个 JUnit case；凭证场景精确绑定 `[rotate]` 和 `[revoke]`，数量缺失与同数量身份替换分别记录 `junit_case_count_mismatch` 和 `junit_case_identity_mismatch`，冻结 case 全部存在时若仍混入额外 testcase 则整份 JUnit 失败关闭。正式证据只允许使用仓库冻结清单，固定 `manifest_ref` 与校验时冻结的 SHA-256 同源，并在清单校验后、恢复诊断后及通过证据原子写入后复核清单未漂移；最后一次复核失败会删除刚写出的证据，Git 修订无法读取时也不能判定整体通过。确认使用冻结清单后，执行器先拒绝与清单、Schema、基线、来源场景或真实测试节点重合的证据路径，并要求所有证据目标都位于当前 `AI_PLATFORM_ROOT/evidence`；仓库外任意文件也不能成为失效或覆盖目标。通过路径守卫的新尝试才会使旧证据失效，替代清单不能删除合法证据。前置、场景或恢复 Runner 抛出的未捕获异常统一收敛为 `runner_exception`，并保证前置或场景异常后仍执行恢复诊断；Runner 返回非法退出码、耗时或原因时统一记为 `runner_invalid_result`，原因码 Schema 使用固定枚举拒绝任意正文。清单校验异常继续保持无正式证据。最小证据为每个场景写入期望/实际 case 数，并在汇总中冻结期望总数 `9`、记录实际总数；证据 Schema 使用八组精确 `contains` 约束，保证每个场景的 ID、能力、样本类别、真实测试节点和期望 case 数各出现一次，重复或替换绑定都会失败；六类样本汇总固定为成功 `1`、失败 `5`、超时 `2`、取消 `1`、恢复 `3`、拒绝 `3`，不能与场景集合漂移。通过态 Schema 另行要求三个检查、八个场景、实际 case `9` 与零失败汇总保持一致。Schema 其余字段只允许清单摘要、场景与样本类别、测试节点、状态、耗时和稳定原因码，JUnit 错误正文、参数正文、结果正文和凭证均不落盘，文件校验后原子替换 `.ai-platform/evidence/p4-12-latest.json`。
- 当前自动验证：执行器、工具契约、工程规则和 Python 注释门禁专项为 `90/90`，覆盖正式 Shell 入口精确函数体、六项执行策略反例、八类场景样本及不变量精确绑定、成功、前置失败、场景失败、恢复失败、前置/场景/恢复 Runner 异常稳定收敛与恢复诊断、非法 Runner 退出码/耗时/原因规范化、固定原因码、通过态一致性、八组精确场景绑定与六类样本汇总 Schema、未知 Git 修订、JUnit 缺失、语法损坏、重复、意外 testcase 扩围、读取异常、裸函数名冲突、参数化数量或身份缩水、场景节点降级替换、替代清单拒绝、证据路径与冻结输入、仓库源码或仓库外任意文件重合拒绝、自定义 `AI_PLATFORM_ROOT`、异常时旧证据失效、运行中及原子写入期间清单漂移、参数化聚合和敏感正文排除；P2/P3/P4 三代联合验收器完整相邻回归 `51/51`，完整非集成组合此前为 `590/590`。冻结清单的八个测试节点经 `pytest --collect-only` 精确收集 `9` 个 case，凭证轮换和撤销分别形成精确 `[rotate]` 与 `[revoke]` case；P4-12 验收器当前收集并通过 `37/37`，全仓当前收集基线为 `809` 项；`bash -n platform` 与 `.venv/bin/python scripts/run_p412_tool_drill.py --help` 同步通过。此前工作树通过全契约 `28/28` 和全仓直接 pytest `801/801`，新增 8 项执行策略、场景样本与不变量绑定反例已纳入本轮专项通过；改动文件 Ruff 与 mypy strict 通过，同一工作树其余部分此前的 Ruff 全仓和 mypy strict `675` 个源文件也已通过。2026-08-17 08:08 CST 使用可访问真实本地依赖的直接全仓 pytest 完成 `809/809`，耗时 330.31 秒；该结果补齐 Python 全仓回归，但不替代正式联合演练证据或统一门禁。原样 `./scripts/verify` 在 Python 前的 Git Diff、全合成数据集、全历史 Secret Scanner、OpenAPI 与契约类型漂移、Registry、SBOM、ReleaseManifest、开发供应链、架构、注释、UnoCSS、React 格式/Lint/类型/`58/58` 测试和生产构建均通过；进入由脚本派生的 Python 子进程后，受限环境拒绝其访问 `127.0.0.1`，因此得到 `594 passed, 8 failed, 194 errors`。该失败集合仍全部来自 PostgreSQL、Valkey、MinIO 或 Tika 连接限制，不作为真实统一门禁结论，也不抵消已有直接测试的通过事实。
- 窄命令真实验证：审批阻塞后依次执行已授权的前置 `./platform doctor`、清单引用的八个真实 PostgreSQL 节点和恢复后 `./platform doctor`；两轮诊断的 Web、API、MinIO、Tika、PostgreSQL、Revision `20260816_0061`、Valkey、5 个 Worker 和 Scheduler 共 13 项均通过，中间八场景因凭证轮换/撤销参数化实际产生 `9/9` 通过。2026-08-17 07:01 CST 和 08:13 CST 又分别独立复验一次 `./platform doctor`，同一 13 项继续全部通过。该结果证明清单所引用故障断言和运行环境当前正常，但没有绕过执行器生成正式证据。
- 当前阻塞：受限环境直接运行正式入口时 `check_runtime` 报告 Docker Engine 不可访问；2026-08-17 08:11 CST 原样请求 `./platform accept-stage-4-tools` 和 08:09 CST 原样请求 `./scripts/verify` 提权，均在项目代码执行前被审批服务 `503 Service Unavailable` 拒绝。用户在获知风险后于 10:54 CST 明确要求继续，两个原样入口随即再次申请提权，仍分别在项目代码执行前被同一审批服务 `503` 拒绝。系统明确禁止通过间接命令绕过，当前仍没有生成 P4-12 正式证据，不能把审批服务或沙箱限制误报为产品通过或失败；继续执行这两个原样入口需要审批服务恢复。
- 下一步：实现按用户指令先行提交；审批服务恢复后仍须原样重试 `./platform accept-stage-4-tools`，确认最小证据原子写入且 Schema 有效，再原样执行可访问真实本地依赖的 `./scripts/verify`。补验通过前不得将节点标记完成。
- 直接提交前收口：最小证据新增每个场景的 `verified_invariants`，八组 Schema 同步精确绑定验证责任并新增重绑定反例；P4-12 验收器 `37/37`、工具契约/工程规则/注释与验收组合 `108/108`、P2/P3/P4 三代验收器 `51/51`，Ruff、格式、mypy strict、Shell 语法和差异检查通过。该结果支持实现提交，不改变正式证据仍缺失的事实。
