...
- 状态：通过。
- 工作流设计与发布：新增 `/workspace/workflows` 工作台和动态菜单入口，支持 Trigger、Condition、Knowledge Retrieval、Model、Approval、Result 六类受限节点的创建、排序、连线、删除和 JSON 配置。草稿可保存并调用后端确定性校验，只有通过校验的草稿才能发布；界面直接展示修订号、校验错误和发布版本，不在前端复制服务端图校验规则。
- 运行监控：支持从当前发布版本创建运行、轮询状态、查看步骤/模型/检索预算和最终输出，刷新后可恢复运行列表。输入与输出继续由后端执行字段级 ABAC 后返回，页面不会从草稿或其他资源推导受限字段；运行列表 API 按独立 `workflow_run_id` 资源复核范围。
- 审批治理：同页提供审批策略创建与不可变修订、我的待办，以及通过、驳回、转交和撤回操作。无管理权限时隐藏工作流创建、编辑、校验、发布和策略治理入口，图编辑控件进入只读状态；菜单裁剪只改善体验，接口仍由后端 PDP 独立拒绝越权请求。
- 菜单与迁移：资源注册表升级至版本 15，共 76 项 Permission、9 个 Page、101 个 API、93 个 Menu 和 94 个 Binding。Revision `20260815_0035` 启用工作流页面和动作菜单、补充运行列表接口绑定，并基于各空间当前正式快照复制生成新版本；已有历史快照不可变且不被原地修改。Migration 升级和降级拆成单条 SQL 执行，兼容 psycopg prepared statement 限制。
- 自动验收：统一 `./scripts/verify` 全部通过，包括 React `31/31`、Python `411/411`、Ruff、mypy strict（415 个源文件）、注释、UnoCSS、架构、OpenAPI/生成契约、权限注册表、Secret Scanner、SBOM、许可证、ReleaseManifest、供应链、契约兼容和生产构建。Migration `base → head → base → head`、菜单快照升级/降级及历史不可变回归 `4/4` 通过。
- 个人空间浏览器验收：使用全合成个人账号完成创建、保存、校验、发布和运行，最终运行成功，执行 2 个节点、0 次模型调用、0 次检索，输入输出正确且刷新后运行记录恢复；审批策略创建成功，待办通过后数量由 1 变为 0。
- 企业空间与视觉验收：Registry 15 正式菜单快照生效；全合成企业普通成员可读取 1 条工作流并成功运行，越权创建返回 `403 POLICY_DENIED`，创建、编辑、校验、发布和策略治理按钮均隐藏。1440×900 桌面视口无横向溢出；390×844 移动视口页面宽度稳定为 390，表格只在局部容器滚动，移动抽屉与路由可用，浏览器控制台无错误。
- 容器与边界：`./platform doctor` 确认 Web、API、MinIO、Tika、PostgreSQL、数据库 Revision `20260815_0035`、Valkey 和 Worker 八项诊断全部通过。真实模型供应商、Linux、容量、镜像扫描和 AI 质量继续保持 `not_configured`/`not_run`；不扩展 SaaS、Go 运行层、真实多源连接器、LLM Grading、多模态图片问答、Channel Gateway 或 Durable Run。
- 提交：`c1d16fa`。

### P1G-01 版本化合成企业数据集

- 状态：通过。
- 固定数据规模：新增 `p1g01-v1` Golden Fixture，包含 1 个合成企业空间、1 个合成个人空间、100 个企业成员、7 个部门、8 个知识库、360 份企业文档和 4 份个人文档。企业成员固定为 97 个活动成员与 3 个停用成员，覆盖企业所有者、管理员、部门负责人、普通员工、知识管理员、审批人、外部协作者和个人空间所有者。
- 文档与权限状态：企业文档包含 280 份当前发布、24 份新旧版本冲突、16 份过期内容、16 份草稿、12 份安全隔离和 12 份软删除样本，并覆盖 `PUBLIC`、`INTERNAL`、`CONFIDENTIAL`、`RESTRICTED` 四级密级、工作空间/部门/个人可见范围和结构化受限字段。全部文档来源固定为本地合成 Markdown，不接入真实 DataSource 或外部网络。
- 联合场景：固定 16 类个人/企业验收入口，覆盖个人与企业主流程、菜单和接口权限、跨空间与部门越权、字段遮罩、旧版本引用、间接 Prompt Injection、SSE 断点恢复、工作流审批恢复、驳回/转交/超时、模型主备降级和配额拒绝；异常场景引用当前稳定错误目录，安全回归继续关联 `p0-11-v1`。
- 可重复性与敏感边界：生成器使用固定命名空间 UUID、固定时间和确定性排序，不读取系统时间、随机数、网络、数据库或真实资料。固定内容摘要为 `f882d5bf1d52b2b96d636468fd4db24c55e15fb257b3b9a0f1f9005e0b313617`；`./scripts/verify` 新增逐字节漂移检查。Fixture 不保存密码、API Key、手机号或证件号，后续装载器只能通过 `p1g01-local-synthetic` 本地配置注入测试凭证。
- 自动验收：专项 `tests/test_p1g01_synthetic_dataset.py` `8/8` 通过，覆盖生成一致性、摘要、规模、合成标记、组织和角色、引用完整性、文档状态、安全级别、授权场景语义、稳定错误码和凭证隔离。统一 `./scripts/verify` 全部通过，包括 React `31/31`、Python `419/419`、Ruff、mypy strict（417 个源文件）、注释、UnoCSS、架构、契约、权限注册表、Secret Scanner、SBOM、许可证、ReleaseManifest、供应链和生产构建。
- 容器与边界：`./platform doctor` 确认 Web、API、MinIO、Tika、PostgreSQL、数据库 Revision `20260815_0035`、Valkey 和 Worker 八项诊断全部通过。本节点只冻结可供后续装载和断言的数据图，不把生成 Fixture 等同于真实 HTTP 联合验收；实际装载、越权、恢复和模型故障注入进入 `P1G-02`。容量、Linux、真实供应商和 AI 质量继续保持 `not_run`/`not_configured`，不扩展既定后置能力。
- 提交：`a4a7ae6`。

### P1G-02 联合越权与恢复验收

- 状态：通过，`core_functional=passed`。
- 固定数据装载：新增只面向测试和本地验收的装载器，写库前强制校验 `p1g01-v1`、全合成标记和 SHA-256 内容摘要；在随机隔离 PostgreSQL Schema 中从空库迁移至 `head`，再以单一事务按外键顺序装入 100 个账号、101 条工作空间成员关系、7 个部门、104 条成员部门关系、11 个角色、110 条角色绑定、364 份文档、388 个不可变版本和 324 个当前发布指针。测试密码只在内存生成 Argon2id Hash，不写回 Fixture、日志或返回值。
- 数据与授权边界：实际数据库断言确认 3 个停用企业成员、个人/企业文档分别固定在各自工作空间、软删除文档和非发布版本不存在当前发布指针，错误版本、非合成记录和摘要漂移在写库前失败关闭。装载器不进入生产启动、不写审计或 Outbox，也不接入真实客户资料或外部数据源。
- 版本化联合清单：新增 `p1g02-v1` 验收清单和统一运行入口，将 `p1g01-v1` 的 16 类个人/企业场景绑定到现有真实 PostgreSQL/HTTP 与领域测试，覆盖停用成员、部门和字段 ABAC、跨空间访问、旧版本引用、Prompt Injection、SSE `Last-Event-ID`、工作流审批恢复、驳回/转交/超时、模型主备和配额拒绝。清单同时强制执行完整 `p0-11-v1` 数据契约、稳定错误目录以及可信 Trace/Request ID 回传，引用不存在的 pytest 节点会在运行前被拒绝。
- 自动验收：`scripts/run_p1g02_acceptance.py` 联合验收 `33/33` 通过；其中数据装载专项 `4/4`、清单契约 `2/2`、`p0-11-v1` 数据契约 `6/6`，其余为按场景去重后的安全与恢复测试。统一 `./scripts/verify` 全部通过，包括 React `31/31`、Python `425/425`、Ruff、mypy strict（422 个源文件）、注释、UnoCSS、架构、OpenAPI/生成契约、权限注册表、Secret Scanner、SBOM、许可证、ReleaseManifest、供应链、契约兼容和生产构建。
...
### P1G-04 全业务 UI/UX、响应式与可访问性验收

- 状态：通过。
- 正式页面：总览、成员、组织、知识生产、知识问答、工作流、运行状态和模型治理共 8 个正式页面在 `390×844` 下均只有一个一级标题，页面级错误、无权限和个人空间状态同样使用 `h1`，嵌套状态继续使用 `h2`，未发现标题层级倒置。
- 响应式与触控：`375×812`、`390×844`、`768×1024`、`1024×768`、`1440×900` 和 `844×390` 六类视口均无页面级横向溢出。Open API Switch 保留 `44×22px` 视觉轨道并通过外层命中区达到 `44×44px`，动态内容不会改变控件布局尺寸。
- 键盘与辅助技术：应用壳层增加“跳到主要内容”链接；深链、菜单和路由切换完成后焦点统一落到 `main-content`，移动抽屉关闭不会把焦点错误恢复到导航按钮。工作空间选项使用业务名称作为可访问名称，不暴露 UUID；密码可见性按钮和图标控件均有可读名称。
- 主题与对比度：主品牌色固定为森林绿 `#176b52`，导航背景固定为深绿 `#12332b`，原荧光绿替换为克制蓝绿色 `#85b7c5`，辅助文字调整为 `#596a63`。辅助文字对 Canvas 对比度为 `5.23:1`、对白色为 `5.73:1`，强调色对深色导航为 `6.24:1`；全局焦点使用白色内描边与品牌色外环，避免焦点在明暗背景上消失。
- 状态与权限：Readiness `503` 的结构化响应可区分 API 不可达和依赖降级，PostgreSQL、Valkey、MinIO 与 Tika 使用中文友好名称。临时授予平台管理员后模型治理页可访问；撤销后同一会话立即显示一级标题拒绝状态，合成账号最终已恢复为 `revoked`，验证前端裁剪不能绕过后端实时授权。
