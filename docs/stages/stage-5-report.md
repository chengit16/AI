...
- 正式 UI 与运行验收：通过 `./platform start` 以仓库绝对数据根重建最终 API、Web、Migration、Worker 与 Scheduler 镜像，Revision `20260820_0071` 的 13 项 doctor 全部通过。正式 Agent 控制台创建“P5-04 知识范围验收助手”，把目标知识库冻结到草稿 `r2`，确定性测试 `5/5`、个人所有者审批和 Release 发布均成功。`1440×900` 与 `390×844` 页面级 `scrollWidth` 分别为 `1440` 与 `390`，知识范围编辑器、Release 列表和移动主导航无横向溢出或文字遮挡，应用自身控制台 warning/error 为空。
- 自定义 Release 只读审计：Release `da5a60f7-499b-4e48-8bc9-8429112861dd` 为 `custom/released`，来源草稿 revision 为 2，运行配置为 V6 `e7f132f7-ec31-495c-b21e-2bc2f231564c`。其不可变快照 `configuration.knowledge_scope_version_ids` 恰有一项 `5c410ab0-da92-5feb-8c3d-9cb605c24182`，范围条目也恰有一项目标知识库 `ee0f2b61-e5ba-426e-972e-04e7f4bedcf1`；该 Release 的 Assistant Run 计数为 0，本轮没有调用供应商、读取正文或凭证。
...
- 验收结论：`blocked`。本地确定性机制、正式范围绑定入口、专项门禁、统一入口、最终镜像、浏览器和自定义 Release 冻结范围审计均已通过；2026-08-20 已补充政策 V11、V6 固定模型参数和一次成功 Invocation，但历史调用的模型上下文越出用户限定的单 PDF 授权范围，不能形成合格质量样本。新 Release 尚未运行，固定网络区域/窗口、另行授权的有界隔离复验与足够样本仍未满足，因此 `P5-04` 继续不标记完成。
...
- 正式绑定与发布续验：当前树为 Agent 草稿增加知识库检索多选和原子范围冻结，服务端批量回读范围摘要并在同事务复核权限、创建不可变范围、更新 revision、失效候选、审计和 Outbox。原样统一门禁 React `63/63`、Python `941/941` 及全部工程检查通过；正确数据根下最终镜像和 Revision `20260820_0071` 的 13 项 doctor 通过。桌面/移动浏览器通过正式 UI 冻结唯一目标知识库并完成 `5/5` 测试、审批和发布；自定义 Release `da5a60f7-499b-4e48-8bc9-8429112861dd` 只读审计确认唯一范围成员为 `ee0f2b61-e5ba-426e-972e-04e7f4bedcf1`，Assistant Run 计数为 0。本轮没有再次调用供应商。
...
- 自定义 Agent Release 已通过正式发布和只读元数据证明冻结唯一目标知识库。下一项可执行交接为另行取得明确授权后，只针对 Release `da5a60f7-499b-4e48-8bc9-8429112861dd` 执行一次有界复验；当前没有再次调用供应商的授权。API Key 继续只由服务端加密边界使用，不进入对话、日志、文档或 Git。
