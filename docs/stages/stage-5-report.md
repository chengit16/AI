...
- 隔离范围重新发布：正式 UI 清除旧选择后选中只含目标 PDF 的隔离知识库 `edf6a10a-d877-4232-9edb-ada5b2caab74`，保存草稿 `r3` 后显示范围版本 `a383ae74-634b-52f9-b4e6-82900e426a50`。确定性测试 `5/5`、个人所有者审批通过，v2 Release `89b32a81-03e3-49cd-892f-0c76a1ea3f26` 已发布。随后限定为 Release 元数据、范围、知识库和文档标题及 Run 计数的 PostgreSQL 只读审计，在 Docker 或数据库访问前被审批服务断连拒绝；因此尚未形成 v2 不可变快照与零 Run 的数据库证据，本轮没有调用供应商、读取正文或凭证。
