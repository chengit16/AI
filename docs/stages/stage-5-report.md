...
- 正式范围绑定入口：复用草稿更新 API 的可选 `knowledge_scope` 字段，不新增旁路写接口。Application 在同一事务内复核工作空间、可见性和密级，创建内容寻址的不可变范围版本，替换草稿 `knowledge_scope_version_ids`，推进 revision、失效旧候选并写入审计与 Outbox；越权、并发或配置失败整体回滚。Agent 列表批量回读脱敏范围摘要，前端可检索多选并在刷新后恢复选择；无 `knowledge.base.read` 或知识库清单加载失败时禁止写入旧缓存。
- 范围绑定专项与统一门禁：前端专项 `6/6`、Agent 契约 `13/13`、Agent 单元 `15/15`、P3-03 PostgreSQL `5/5`、合并专项 `20/20` 通过；Ruff、前端 Lint/TypeScript、契约生成与漂移、Agent 模块 mypy strict 均通过。沙箱外原样 `./scripts/verify` 最终为 React `63/63`、Python `941/941`，mypy strict `754` 个源文件及全部工程检查通过。
- 正式 UI 与运行验收：通过 `./platform start` 以仓库绝对数据根重建最终 API、Web、Migration、Worker 与 Scheduler 镜像，Revision `20260820_0071` 的 13 项 doctor 全部通过。正式 Agent 控制台创建“P5-04 知识范围验收助手”，首次把知识库 `ee0f2b61-e5ba-426e-972e-04e7f4bedcf1` 冻结到草稿 `r2` 并发布 v1；后续交叉审计发现该库除目标 PDF 外还包含“前端面试题”，所以 v1 不能用于单 PDF 复验。`1440×900` 与 `390×844` 页面级 `scrollWidth` 分别为 `1440` 与 `390`，知识范围编辑器、Release 列表和移动主导航无横向溢出或文字遮挡，应用自身控制台 warning/error 为空。
- 隔离范围重新发布：正式 UI 清除旧选择后选中只含目标 PDF 的隔离知识库 `edf6a10a-d877-4232-9edb-ada5b2caab74`，保存草稿 `r3` 后显示范围版本 `a383ae74-634b-52f9-b4e6-82900e426a50`。确定性测试 `5/5`、个人所有者审批通过，v2 Release `89b32a81-03e3-49cd-892f-0c76a1ea3f26` 已发布。用户于 2026-08-21 再次明确授权只查询 Release 元数据、知识范围、知识库和文档标题及 Run 计数；审批服务仍在 Docker 或数据库访问前返回 `503 Service Unavailable`，请求 ID 为 `a52abf78-3e1b-4eb6-9830-f05e19ef38c1`。因此尚未形成 v2 不可变快照与零 Run 的数据库证据，本轮没有访问数据库、调用供应商、读取正文或凭证，也没有改走间接路径绕过审批。
- 启动路径校正：直接调用 Compose 曾把 `.env` 中相对 `AI_PLATFORM_ROOT` 解析到 `infra/compose/.ai-platform`，只读查询因此得到空库；未在该错误实例写入业务数据。随后改用仓库唯一入口 `./platform start`，并通过容器挂载路径、目标知识库只读查询和 13 项 doctor 确认最终实例已恢复到仓库根 `.ai-platform`。后续容器操作继续以 `./platform` 为入口。
- 验收结论：`blocked`。本地确定性机制、正式范围绑定入口、专项门禁、统一入口、最终镜像和浏览器发布流水均已通过；2026-08-20 已补充政策 V11、V6 固定模型参数和一次成功 Invocation，但历史调用的模型上下文越出用户限定的单 PDF 授权范围，不能形成合格质量样本。v2 Release 的数据库只读审计、固定网络区域/窗口、另行授权的有界隔离复验与足够样本仍未满足，因此 `P5-04` 继续不标记完成。
...
- 隔离复验执行待审计：用户再次明确授权把目标 PDF 命中的必要 `INTERNAL` 片段发送到 `https://ai.input.im/responses`，并接受保留 3650 天且禁止训练。执行前确认 API 容器健康，容器内两处 Release 知识范围实现的 SHA-256 与工作树完全一致；目标 PDF 在宿主机和容器内的 SHA-256 均为 `259a5013f325f1dcfbedb519b5820fda8f375ed3e2490228f8fae3a81192f696`，一次性脚本通过宿主机 Ruff、格式和语法检查及容器语法检查。脚本只启动一个进程，并在约 70 秒后以退出码 0 结束，但执行工具没有回传脚本预期的 JSON 结果；随后只读查询本地 PostgreSQL 的申请在命令启动前被审批服务自身的 `503 Service Unavailable` 拒绝，请求 ID 为 `118789d2-8263-4e71-aa8c-33687534e0be`。当时无法证明供应商调用是否发生及其范围、Attempt、Token、引用和回答事实，因此本次授权先按“可能已消耗”处理，禁止自动重跑，等待审批恢复后仅做数据库只读审计。
- 隔离复验首层只读审计：审批短暂恢复后，只读 PostgreSQL 元数据确认 `2026-08-20 07:54–08:22 UTC` 同一 Release `79f78789-6e5a-4a7a-a88f-e4fac907f4f1` 与 V6 下共有 9 个 Run，其中 8 个形成 `succeeded` Invocation，均使用 `gpt-5.6-sol`、单次 Attempt、无降级，输入/输出 Token 合计为 `51,238/1,462`；另 1 个完成 Run 没有 Invocation。该事实证明供应商调用已经发生，不能继续按“可能未调用”处理；成功 Invocation 数量也超过“单次问答”的字面范围，后续禁止自动重跑。当时第二层只读查询在进入 Docker 前被审批服务自身的 `503 Service Unavailable` 拒绝，尚不能判断 Release 类型与引用范围。
- 指定 Release 只读复核结论：用户明确限定只读核对知识范围、文档标题、Provider Request ID 和完成态引用，并禁止读取消息正文、检索片段正文、凭证、供应商和验收脚本。最终 PostgreSQL 审计确认该 Release 为版本 4 的 `system` Release，`snapshot` 没有 `knowledge_scope_version_ids`，范围版本和范围条目均为 0；全部引用来自同一知识库 `ee0f2b61-e5ba-426e-972e-04e7f4bedcf1`。8 次成功 Attempt 的 Provider Request ID 字段均非空且 8 个值互不相同，每个字段当前表现为两个逗号分隔的 UUID 形标识；39 条完成态引用中目标 PDF 为 32 条、范围外“前端面试题”为 7 条，7 个成功 Run 发生范围外引用，只有 1 个成功 Run 的 4 条引用全部来自目标 PDF。由于该唯一表面干净的 Run 仍绑定无知识范围的系统 Release，8 次调用全部不能作为自定义 Agent 隔离通过样本；另 1 个完成 Run 没有 Invocation，证据集为 `uncertain` 且无引用。本次仅执行 `SELECT`，没有读取正文或凭证、没有调用供应商、没有写入数据库，也没有重跑验收。
...
- 正式绑定与发布续验：当前树为 Agent 草稿增加知识库检索多选和原子范围冻结，服务端批量回读范围摘要并在同事务复核权限、创建不可变范围、更新 revision、失效候选、审计和 Outbox。原样统一门禁 React `63/63`、Python `941/941` 及全部工程检查通过；正确数据根下最终镜像和 Revision `20260820_0071` 的 13 项 doctor 通过。第一次 UI 发布的 v1 Release 只读审计虽确认唯一范围成员为知识库 `ee0f2b61-e5ba-426e-972e-04e7f4bedcf1`、Assistant Run 计数为 0，但后续发现该知识库包含目标 PDF 与“前端面试题”，不能用于单 PDF 隔离复验。正式 UI 已把草稿 `r3` 改绑到隔离知识库 `edf6a10a-d877-4232-9edb-ada5b2caab74`，完成 `5/5` 测试和审批并发布 v2 Release `89b32a81-03e3-49cd-892f-0c76a1ea3f26`；v2 的 PostgreSQL 只读审计在命令启动前被审批服务断连拒绝，本轮没有再次调用供应商。
- v2 审计审批链复核：2026-08-21 读取非敏感配置键确认 `~/.codex/config.toml` 已为 `approvals_reviewer = "user"`；但当前任务运行时仍显示启动时载入的 `auto_review`。沙箱内 `docker ps` 因 Docker Socket 权限被拒绝，随后同一只读命令的提权申请在进程创建前由自动审核请求 `503 Service Unavailable` 拒绝，请求 ID 为 `081234b6-d1b3-49fd-80fc-6bf1e944e4ce`。本轮未访问 Docker、PostgreSQL 或供应商，也未读取正文或凭证。要使永久配置生效，需重启 Codex 或新建任务后重新执行同一审计；旧任务中继续重试不会改变结论。
...
- `P5-04` 本地机制和 V6 真实供应商链路已经运行，但隔离复验错误绑定无知识范围的系统 Release，8 次成功调用中 7 次发生授权范围偏差，全部不能计入通过样本。自定义 Agent Release 知识库范围修复完成后，节点仍因真正绑定目标范围的合格复验、固定网络区域/窗口和足量授权样本缺失而保持受阻。
- 正式 UI 已发布绑定隔离知识库的 v2 Release `89b32a81-03e3-49cd-892f-0c76a1ea3f26`，但其 PostgreSQL 只读审计尚未通过审批。下一项先只读确认不可变范围、唯一文档和零 Run；完成后仍须另行取得明确外传授权，才能只针对该 v2 Release 执行一次有界复验。API Key 继续只由服务端加密边界使用，不进入对话、日志、文档或 Git。
- `P5-04` 的真实质量结论已具备审核通过的真实供应商和单次验收配置，但本次隔离复验审计已经完成且判定不通过；仍需要正确绑定自定义 Release 的复验、固定网络区域、窗口和达到下限的授权样本。这些运行事实形成前不能标记真实质量目标通过。
