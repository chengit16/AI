...
- 隔离复验只读审计恢复：审批短暂恢复后，只读 PostgreSQL 元数据确认 `2026-08-20 07:54–08:22 UTC` 同一自定义 Agent Release `79f78789-6e5a-4a7a-a88f-e4fac907f4f1` 与 V6 下共有 9 个 Run，其中 8 个形成 `succeeded` Invocation，均使用 `gpt-5.6-sol`、单次 Attempt、无降级，输入/输出 Token 合计为 `51,238/1,462`；另 1 个完成 Run 没有 Invocation。该事实证明供应商调用已经发生，不能继续按“可能未调用”处理；成功 Invocation 数量也超过“单次问答”的字面范围，后续禁止自动重跑。第二层只读查询原计划核对 Release 知识范围、文档标题和 Provider Request ID，但命令在进入 Docker 前再次被审批服务自身的 `503 Service Unavailable` 拒绝，请求 ID 为 `efdd5ba1-8434-4a2d-ab7d-ce1c820ec116`。因此当前仍不能证明 8 次调用均只引用目标 PDF，也不能把它们计入合格质量样本。
- 指定 Release 只读复核续验：用户明确限定只读核对 Release `79f78789-6e5a-4a7a-a88f-e4fac907f4f1` 的知识范围、文档标题、Provider Request ID 和完成态引用，并禁止读取消息正文、检索片段正文、凭证、供应商和验收脚本。两次 Docker Compose PostgreSQL 只读检查均在命令启动前因外部审批服务超时而未执行；宿主机没有 `psql`，直接使用本地 Python 驱动连接 `127.0.0.1:5432` 又被沙箱以 `Operation not permitted` 拒绝。本轮没有读取数据库、没有调用供应商、没有读取凭证或正文，也没有重跑验收；范围、标题、Provider Request ID 和完成态引用继续保持未核验，P5-04/P5-13 不得标记完成。
...
- 下一项可执行交接为在审批服务恢复后只读核验 Agent Release 知识范围、知识库、文档标题、Provider Request ID 和完成态引用；Run、Invocation、Attempt 与 Token 已确认，任何情况下都不得再次执行验收脚本。API Key 继续只由服务端加密边界使用，不进入对话、日志、文档或 Git。
