...
- `P4-11` 节点前数据库基线 Revision 为 `20260816_0060`；阶段 3 本地 Agent 平台版本为 `0.3.0`，ReleaseManifest 摘要为 `60e5d17d…73e7c81`。
...
截至 2026-08-17，阶段 2 和阶段 3 依赖已经通过并冻结，`P4-01`～`P4-11` 已完成并通过各自门禁，当前执行 `P4-12` 重复投递、Adapter 超时、响应丢失、凭证撤销、审批过期、取消竞态、Worker 重启和跨空间联合演练。固定清单、最小证据契约和统一入口已实现，Shell 包装层整个函数体已精确冻结为 6 条非空行，只能按顺序完成运行时检查、环境准备、本地服务设置导出并调用唯一正式执行器及其固定参数，不能追加命令或扩大范围；执行策略的六项参数化反例进一步保证前置/恢复命令、前置失败停止、始终恢复诊断、失败关闭模式和证据路径不能放宽，每个能力的样本类别与验证不变量也在清单加载阶段精确绑定，不能用总并集完整掩盖单场景虚增样本或减少验证责任。窄命令下前后 13 项诊断和真实 PostgreSQL 参数化场景 `9/9` 已通过，场景 ID、P4-01 来源和 PostgreSQL 节点三元绑定、JUnit 裸函数名唯一性、8 场景对应 9 case 的数量及 `[rotate]`/`[revoke]` 身份冻结、额外 testcase 拒绝、证据 Schema 八组精确场景绑定与六类样本精确汇总、通过态的诊断/场景/汇总一致性、清单摘要三次复核与写入竞态删除、证据目标严格限定到当前 `AI_PLATFORM_ROOT/evidence`、Runner 异常恢复与返回值规范化、固定原因码和未知 Git 修订阻断也已加入失败关闭校验；审批服务 `503 Service Unavailable` 尚未解除，正式入口未生成证据且原样统一门禁未通过前节点保持进行中。
