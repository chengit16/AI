...
- 阶段 4 尚未关闭：`P4-12` 实现提交为 `817abd5`，但正式联合演练证据和原样统一门禁仍受审批服务 `503 Service Unavailable` 阻塞，`P4-13` 与 `stage-4-complete` 标签均未完成。
- 用户于 2026-08-17 明确要求先提交 P4-12 实现并进入阶段 5，因此允许先完成 `P5-01` 计划与入口审计；`P5-02`～`P5-13` 的功能实现不得绕过阶段 4 关闭门禁。
...
`P5-01` 只冻结范围、事实源、门禁和外部条件，可以在阶段 4 受阻期间完成。`P5-02`～`P5-13` 必须等待 `P4-12`、`P4-13`、阶段 4 报告和 `stage-4-complete` 标签全部完成；需要真实模型或价格结论的节点还必须等待供应商审核、固定模型版本、固定网络区域和足够样本。
