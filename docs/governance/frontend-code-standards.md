...
`scripts/check-frontend-comments.mjs` 使用 TypeScript AST 检查全部当前手写源码，至少覆盖：文件头 `@description`、导出声明 JSDoc、Props/Options 字段 JSDoc、TODO/FIXME/HACK 格式以及生成文件排除边界。检查接入 `./scripts/verify`，任何当前文件违规都阻断节点验收，不保留历史告警白名单。
