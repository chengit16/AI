...
- 不采用“每 N 行必须有注释”的机械指标，也不为简单赋值、普通 JSX 和显而易见的分支添加低价值注释。

### 13.2 文件级职责说明

以下文件必须在实现前提供模块级职责说明：页面入口 `pages/**/index.tsx`、业务 Hook、API Client 和 Service、Zustand Store、公共组件、路由与菜单权限配置、SSE/缓存/错误基础设施以及跨模块工具。

模块说明至少回答“负责什么、不负责什么、依赖的业务边界是什么、有哪些权限或副作用约束”，推荐格式如下：
...
跨文件使用的 React 组件、Props 类型、自定义 Hook、Hook 返回对象、API Service、公共工具、Store、跨模块常量和复杂配置必须提供 JSDoc。说明应覆盖用途、关键参数、返回结果、权限边界、错误模式或调用前置条件；只在当前文件使用的私有小组件和简单类型不强制逐字段说明。
...
Props 中非直观字段必须说明，特别是“只控制体验、不构成安全边界”的权限属性：
...
以下逻辑必须在职责最近的位置增加中文块注释，并说明“为什么这样做”：
...
行内注释只用于临界值、特殊兼容、安全限制、并发控制、性能优化或防止未来误删的代码。

### 13.5 文件类型最低要求

| 文件类型 | 最低注释要求 |
| --- | --- |
| 页面入口 | 文件职责、权限来源、数据来源和主要状态入口 |
| 业务 Hook | Query/Mutation 关系、状态来源、缓存刷新原因和清理策略 |
| API Service | 请求用途、参数语义、返回结果、错误和安全边界 |
| API Client | 会话、CSRF、工作空间、错误转换和取消策略 |
| 公共组件 | 组件职责、非直观 Props、回调时机和权限限制 |
| Zustand Store | 状态归属、持久化方式、重置时机和跨页面影响 |
| 菜单/权限配置 | 菜单、页面、接口和权限码的绑定关系 |
...
阶段 1 建立轻量检查，至少覆盖：新增或修改的跨文件导出是否有 JSDoc；高风险文件是否有模块职责说明；TODO/FIXME 是否包含责任角色、日期和退出条件；新增注释是否使用中文；是否出现低价值模板注释、敏感信息或误改生成文件。

门禁采用渐进策略：新增文件和本次修改文件按阻断处理，历史遗留先告警；不为了通过检查批量翻译无关历史注释。每个节点在测试和 Git 提交前执行该检查。
